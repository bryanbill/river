...
impl<'a> Lexer<'a> {
    ...
    fn read_ident_or_keyword(&mut self, start: usize, first_char: char) -> Spanned<Token> {
        ...
        match lower.as_str() {
            ...
            "alter" => Spanned::new(Token::Alter, start..end),
            "add" => Spanned::new(Token::Add, start..end),
            "column" => Spanned::new(Token::Column, start..end),
            "drop" => Spanned::new(Token::Drop, start..end),
